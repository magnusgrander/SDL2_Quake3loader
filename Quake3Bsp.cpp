...
const float kMaxStepHeight = 22.0f;
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	m_vTraceMins = glm::vec3(0, 0, 0);
	m_vTraceMaxs = glm::vec3(0, 0, 0);
	m_vExtents   = glm::vec3(0, 0, 0);
	
	// This will store the normal of the plane we collided with
	m_vCollisionNormal = glm::vec3(0, 0, 0);
...
void CQuake3BSP::ChangeGamma(char *pImage, int size, float factor)
{
...
void CQuake3BSP::CreateLightmapTexture(GLuint &texture, char *pImageBits, int width, int height)
{
    ...
    // TEST
    // 128 to 256
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 128, 128, 0, GL_RGB, GL_UNSIGNED_BYTE, pImageBits);
...
bool CQuake3BSP::LoadBSP(const char *strFileName)
{
	...
	if((fp = fopen(strFileName, "rb")) == NULL)
	{
		...
		printf("Could not find BSP file!");
		return false;
	}
	...
	if(lumps[kVisData].length) 
	{
		...
		fread(&(m_clusters.charsPerCluster), 1, sizeof(int), fp);

		// Allocate the memory for the cluster bitsets
		int size = m_clusters.numOfClusters * m_clusters.charsPerCluster;
...
int CQuake3BSP::FindLeaf(const glm::vec3 &vPos)
{
	int i = 0;
	float distance = 0.0f;
...
inline int CQuake3BSP::IsClusterVisible(int current, int test)
{
	...
	char visSet = m_clusters.pBitsets[(current*m_clusters.charsPerCluster) + (test / 8)];
...
float Dot(glm::vec3 vVector1, glm::vec3 vVector2) 
{
	//    (V1.x * V2.x        +        V1.y * V2.y        +        V1.z * V2.z)
	return ( (vVector1.x * vVector2.x) + (vVector1.y * vVector2.y) + (vVector1.z * vVector2.z) );
}

glm::vec3 CQuake3BSP::TryToStep(glm::vec3 vStart, glm::vec3 vEnd)
{
	...
	for(float height = 1.0f; height <= kMaxStepHeight; height++)
	{
		...
		glm::vec3 vStepStart = glm::vec3(vStart.x, vStart.y + height, vStart.z);
		glm::vec3 vStepEnd   = glm::vec3(vEnd.x, vStart.y + height, vEnd.z);
				
		// Test to see if the new position we are trying to step collides or not
		glm::vec3 vStepPosition = Trace(vStepStart, vStepEnd);
		...
		if(!m_bCollided)
		{
			...
			glm::vec3 vNewView = g_Camera.View();					
			g_Camera.SetView(glm::vec3(vNewView.x, vNewView.y + height, vNewView.z));
...
glm::vec3 CQuake3BSP::Trace(glm::vec3 vStart, glm::vec3 vEnd)
{
	...
	else	// Else COLLISION!!!!
	{
		...
		glm::vec3 vNewPosition = vStart + ((vEnd - vStart) * m_traceRatio);
		
		// Get the distance from the end point to the new position we just got
		glm::vec3 vMove = vEnd - vNewPosition;
		...
		glm::vec3 vEndPosition = vEnd - m_vCollisionNormal*distance;
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
		...
		// 
		if(m_vCollisionNormal.y > 0.1f || m_bGrounded)
			m_bGrounded = true;
		else
			m_bGrounded = false;
...
glm::vec3 CQuake3BSP::TraceRay(glm::vec3 vStart, glm::vec3 vEnd)
{
...
glm::vec3 CQuake3BSP::TraceSphere(glm::vec3 vStart, glm::vec3 vEnd, float radius)
{
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	glm::vec3 vNewPosition = Trace(vStart, vEnd);
...
glm::vec3 CQuake3BSP::TraceBox(glm::vec3 vStart, glm::vec3 vEnd, glm::vec3 vMin, glm::vec3 vMax)
{
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	m_vExtents = glm::vec3(-m_vTraceMins.x > m_vTraceMaxs.x ? -m_vTraceMins.x : m_vTraceMaxs.x,
...
void CQuake3BSP::CheckNode(int nodeIndex, float startRatio, float endRatio, glm::vec3 vStart, glm::vec3 vEnd)
{
	...
	else
	{
		...
		glm::vec3 vMiddle;	// This stores the middle point for our split ray
...
void CQuake3BSP::CheckBrush(tBSPBrush *pBrush, glm::vec3 vStart, glm::vec3 vEnd)
{
	...
	for(int i = 0; i < pBrush->numOfBrushSides; i++)
	{
		...
		glm::vec3 vOffset = glm::vec3(0, 0, 0);
...
void CQuake3BSP::RenderLevel(const glm::vec3 &vPos)
{
	...
	while(i--)
	{
		...
		while(faceCount--)
		{
			...
			if(m_pFaces[faceIndex].type == FACE_PATCH || m_pFaces[faceIndex].type == FACE_BBOARD )
              { continue; }
