...
struct tBSPVisData
{
	...
	char *pBitsets;				// The array of chars that holds the cluster bitsets				
};
...
public:
	...
	glm::vec3 TraceRay(glm::vec3 vStart, glm::vec3 vEnd);

	// This traces a sphere along a ray to check for collision with the brushes
	glm::vec3 TraceSphere(glm::vec3 vStart, glm::vec3 vEnd, float radius);

	// This traces a axis-aligned bounding box (AABB) along a ray to check for collision
	glm::vec3 TraceBox(glm::vec3 vStart, glm::vec3 vEnd, glm::vec3 vMin, glm::vec3 vMax);
...
private:
	...
	void CreateLightmapTexture(GLuint &texture, char *pImageBits, int width, int height);
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	glm::vec3 m_vTraceMins;		// This stores the minimum values of the AABB (bottom corner)
	glm::vec3 m_vTraceMaxs;		// This stores the maximum values of the AABB (top corner)
	glm::vec3 m_vExtents;		// This stores the largest length of the box
	glm::vec3 m_vCollisionNormal;	// This stores the normal of the plane we collided with
